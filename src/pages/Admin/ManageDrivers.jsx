...
const DriverForm = ({
  ...
  <form onSubmit={onSubmit} className="space-y-4 pt-2">
    <div>
      <label className="text-[10px] font-black uppercase tracking-widest text-text-muted mb-1.5 block px-1">
        Full Name
      </label>
      <div className="flex items-center gap-3 bg-app-bg border border-border-main rounded-xl px-4 py-3 focus-within:border-brand-primary transition-all">
    ...
    <button
      type="submit"
      disabled={submitting}
      className="w-full flex items-center justify-center gap-2 bg-brand-primary text-white font-black py-4 rounded-xl text-xs uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 mt-2 shadow-lg shadow-brand-primary/20 cursor-pointer"
    >
      <FiSave size={15} />
...
const ManageDrivers = () => {
  ...
  const handleUpdateDriver = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await API.put(`/driver/${selectedDriver._id}`, formData);
      setDrivers((prev) =>
  ...
  const handleDeleteDriver = async () => {
    setSubmitting(true);
    try {
      await API.delete(`/driver/${selectedDriver._id}`);
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {drivers.map((driver) => (
                  <tr key={driver._id} className="border-b border-border-main/50 hover:bg-slate-50/50 transition-colors">
                    <td className="px-8 py-5">
                      <div className="font-black text-text-main uppercase text-sm italic">{driver.fullName}</div>
                      <div className="text-[10px] text-text-muted font-bold mt-0.5 tracking-wider">{driver.email || driver.phone}</div>
                    </td>
                    <td className="px-8 py-5 text-xs font-bold text-text-muted uppercase italic">{driver.vehicleType}</td>
                    <td className="px-8 py-5 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <button onClick={() => openDetails(driver)} className="cursor-pointer p-3 bg-blue-50/50 rounded-xl text-blue-400 hover:text-blue-600 hover:bg-blue-100 transition-all border border-transparent"><FiInfo size={18}/></button>
                        <button onClick={() => openEdit(driver)} className="p-3 bg-amber-50/50 rounded-xl cursor-pointer text-amber-400 hover:text-amber-600 hover:bg-amber-100 transition-all border border-transparent"><FiEdit3 size={18}/></button>
                        <button onClick={() => openDelete(driver)} className="cursor-pointer p-3 bg-red-50/50 rounded-xl text-red-400 hover:text-red-600 hover:bg-red-100 transition-all border border-transparent"><FiTrash2 size={18}/></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {/* MODALS */}
      <Modal isOpen={isModalOpen && modalType === "add"} onClose={closeModal} title="New Enrollment">
        ...
        {selectedDriver && (
          <div className="space-y-3 pt-2">
            {[
              ...
              { label: "Email", value: selectedDriver.email || "N/A", icon: <FiMail size={14} /> },
      ...
      <Modal isOpen={isModalOpen && modalType === "delete"} onClose={closeModal} title="Driver Remove">
        {selectedDriver && (
          <div className="pt-2 space-y-5">
            <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-4 flex gap-3 text-center flex-col items-center">
              ...
              <p className="text-sm font-bold text-text-main uppercase mt-2">Remove {selectedDriver.fullName}?</p>
            </div>
            <div className="flex gap-3">
              <button onClick={closeModal} className="cursor-pointer flex-1 py-4 rounded-xl border border-border-main font-black text-[10px] uppercase tracking-widest text-text-muted">Cancel</button>
              <button onClick={handleDeleteDriver} className="cursor-pointer flex-1 py-4 rounded-xl bg-red-500 text-white font-black uppercase tracking-widest text-[10px] hover:bg-red-600 transition-all shadow-lg shadow-red-500/20">Yes, Remove</button>
            </div>
          </div>
        )}
      </Modal>
